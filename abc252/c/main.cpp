...
/* 足し算ができるpair */
template <typename T, typename U> class my_pair : public std::pair<T, U> {
...
/* 方向に進む用 */
const V<my_pair<ll, ll>> d_pair = {{0, -1}, {1, 0}, {0, 1}, {-1, 0}};
...
int main() {
  ...
  // noLine[数字][n番ライン]
  V<V<bool>> noLine(10, V<bool>(n, false));

  for (ll i = 0; i < S[0].size(); i++) {
    V<bool> noNumber(10, false);

    for (ll j = 0; j < S.size(); j++) {
      ll c = ctoi(S[j][i]);

      if (!noNumber[c] && !noLine[c][j]) {
        mp[c]++;
        noNumber[c] = true;
        noLine[c][j] = true;
      }
