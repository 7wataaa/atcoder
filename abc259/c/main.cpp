...
int main() {
  ...
  deque<CharData> sData;
  sData.pb({'!', 0});
  deque<CharData> tData;
  tData.pb({'!', 0});
  ...
  for (ll i = 0; i < sData.size(); i++) {
    auto sFront = sData.front();
    sData.pop_front();

    auto tFront = tData.front();
    tData.pop_front();

    if (sFront.c != tFront.c) {
      cout << "No" << endl;
      return 0;
    }

    if (!(sFront.count == tFront.count ||
          (sFront.count >= 2 && sFront.count < tFront.count))) {
      cout << "No" << endl;
      return 0;
    }
  }
