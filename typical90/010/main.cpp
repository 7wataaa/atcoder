...
int main() {
  ...
  V<ll> ruiseki2(n + 1, 0);
  for (ll i = 0; i < n; i++) {
    ll c = CP[i].fs, p = CP[i].sc;

    ruiseki1[i + 1] = ruiseki1[i] + (c == 1 ? p : 0);
    ruiseki2[i + 1] = ruiseki2[i] + (c == 2 ? p : 0);
  }

  ll q = llin;
  rep(i, 0, q) {
    ll l = llin, r = llin;
    ll ans1 = ruiseki1[r] - ruiseki1[l - 1];
    ll ans2 = ruiseki2[r] - ruiseki2[l - 1];

    cout << make_pair(ans1, ans2) << endl;
  }
}
